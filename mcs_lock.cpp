...
int MCS_Mutex_create(int tail_rank, MPI_Comm comm, MCS_Mutex *hdl_out)
{
    ...
    hdl = (mcs_mutex_s *)malloc(sizeof(struct mcs_mutex_s));
    assert(hdl != NULL);
...
int MCS_Mutex_lock(MCS_Mutex hdl, int world_size)
{
    ...
    MPI_Win_sync(hdl->window);
    // &shmem_world_rank
    MPI_Fetch_and_op(&world_size, &prev, MPI_INT, hdl->tail_rank, MCS_MTX_TAIL_DISP,
    ...
    if (prev != -1)
    {
        ...
        MPI_Accumulate(&world_size, 1, MPI_INT, prev, MCS_MTX_ELEM_DISP, 1, MPI_INT, MPI_REPLACE, hdl->window);
...
int MCS_Mutex_trylock(MCS_Mutex hdl, int world_size, int *success)
{
    ...
    MPI_Compare_and_swap(&world_size, &nil, &tail, MPI_INT, hdl->tail_rank,
...
int MCS_Mutex_unlock(MCS_Mutex hdl, int world_size)
{
    ...
    MPI_Fetch_and_op(NULL, &next, MPI_INT, world_size,
                     MCS_MTX_ELEM_DISP, MPI_NO_OP, hdl->window);
    MPI_Win_flush(world_size, hdl->window);
    ...
    if (next == -1)
    {
        ...
        MPI_Compare_and_swap(&nil, &world_size, &tail, MPI_INT, hdl->tail_rank,
                             MCS_MTX_TAIL_DISP, hdl->window);
        MPI_Win_flush(hdl->tail_rank, hdl->window);
        ...
        if (tail != world_size)
        {
            ...
            assert(tail >= 0 && tail < world_size);

            for (;;)
            {
                int flag;

                MPI_Fetch_and_op(NULL, &next, MPI_INT, world_size, MCS_MTX_ELEM_DISP,
